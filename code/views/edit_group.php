               ...
               <tr>
                ...
                <tr>
                	<td align="center" class="perm_li">
                        ...
                        <ol class="pages_list">                        	
							<?php 
							//echo "<pre>"; print_r($arr_perm_data);	
									...
									<?php 
                                        ...
                                        <li class="func_li">
											...
											<ol class="subfunc_list" style="display:none">
												...
												?>
                                                <li class="subfunc_li">
                                                	<div style="float:left; <?php if($v1['is_crud'] == 1) { echo"width:25%;"; } ?>"> 
                                                    	<?php
															$checked = '';
															if(isset($groupid) && isset($check_fun))
															{	
                 ...
                 <tr>
                    <td><label for="landing_page">Landing Page <span>*</span></label>
                    <select name="landing_page" id="landing_page" class="validate[required]">
					...
					{	if(isset($arr_perm_data[$j]['functions']) && count($arr_perm_data[$j]['functions'])>0)
                        {
							foreach($arr_perm_data[$j]['functions'] as $k=>$v)
                            {
								foreach($v['subfunction'] as $k1=>$v1)
                                {
									$val = $arr_perm_data[$j]['page_id'].",".$v['function_id'].",".$v1['function_id'];
									if(isset($data['groupDetails']))
									{
										$check_permission = check_permission('', $groupid, $arr_perm_data[$j]['page_id'],$v['function_id'],$v1['function_id'],'');
										if($check_permission)
										{$disabled = "";}
										else
										{ $disabled = "disabled=\"disabled\""; }
										
										if($data['groupDetails']['landing_page']==$val)
										{ $selected = "selected";}
										else
										{$selected = "";}
									}
									...
									echo "<option ".$selected."".$disabled." id=".$val." value=".$val.">".$arr_perm_data[$j]['module_name']."->".$v['friendly_name']."->".$v1['friendly_name']."</option>";
								}
							}
						}
					}
...
$(document).ready(function(){
	...
	{	if($('input[name=check_all_pages]').is(":checked"))
		{
			$("#landing_page option").each(function(){
				$(this).removeAttr("disabled");
				$("#landing_page").val("").change();	
			});
	...
	{	
		...
		var val = pageid +','+ functionid +','+ subfunctionid;
		
		$("#landing_page option").each(function(){
		  if($(this).val() == val){
			  if($(my_check_box).is(":checked"))
			  {	if($(my_check_box).val() == subfunctionid)
			  	{$(this).removeAttr("disabled"); }
			  }
			  else
			  { if($(my_check_box).val() == subfunctionid)
